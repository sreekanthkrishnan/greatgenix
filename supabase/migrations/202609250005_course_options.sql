-- Reuse course color for the built-in artwork selection; optional metadata is stored as empty text.
...
declare t text:=p_action->>'type'; d jsonb; c uuid; target uuid; l public.lessons; begin
 ...
 if t in ('course','course-access') then
  ...
  if t='course' then
   ...
   insert into public.courses(id,"orgId",title,subject,grade,batch,description,color,"teacherId") values(c,p_org,trim(d->>'title'),coalesce(trim(d->>'subject'),''),coalesce(trim(d->>'grade'),''),coalesce(trim(d->>'batch'),''),d->>'description',coalesce(d->>'color','sage'),(d->>'teacherId')::uuid);
  ...
  update public.courses set
   color=case when d ? 'color' then d->>'color' else color end,
   subject=case when d ? 'subject' then coalesce(trim(d->>'subject'),'') else subject end,
   grade=case when d ? 'grade' then coalesce(trim(d->>'grade'),'') else grade end,
   batch=case when d ? 'batch' then coalesce(trim(d->>'batch'),'') else batch end
  where id=c and "orgId"=p_org;
