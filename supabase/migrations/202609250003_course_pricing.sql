-- Existing courses retain unknown prices until a teacher saves their pricing.
-- Keep the legacy paymentInstructions column for compatibility; the UI no longer uses it.
alter table public.courses add column "coursePrice" numeric;
alter table public.courses add column "discountedPrice" numeric;
alter table public.courses add constraint course_price_valid check (
 "coursePrice" is null or ("coursePrice" > 0 and "coursePrice" < 'Infinity'::numeric)
);
alter table public.courses add constraint course_discount_valid check (
 "discountedPrice" is null or ("coursePrice" is not null and "discountedPrice" > 0 and "discountedPrice" < "coursePrice")
);
