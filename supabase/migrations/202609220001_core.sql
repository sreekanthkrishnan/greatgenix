...
create table public.platform_admins ("userId" uuid primary key references auth.users(id));
...
create table private.video_assets ("lessonId" uuid primary key references public.lessons(id), "uploadId" text unique, "assetId" text unique, "playbackId" text, "uploadUrl" text);
...
create table private.webhook_events (id text primary key, "createdAt" timestamptz not null default now());
create function private.is_platform() returns boolean language sql stable security definer set search_path='' as $$ select exists(select 1 from public.platform_admins where "userId"=auth.uid()) $$;
create function private.org_role(o uuid) returns text language sql stable security definer set search_path='' as $$ select m.role from public.memberships m join public.organizations g on g.id=m."orgId" where m."orgId"=o and m."userId"=auth.uid() and m.active and g.active $$;
create function private.is_admin(o uuid) returns boolean language sql stable security definer set search_path='' as $$ select coalesce(private.org_role(o)='teacher-admin',false) $$;
create function private.feature(o uuid,f text) returns boolean language sql stable security definer set search_path='' as $$ select coalesce((select active and (features->>f)::boolean from public.organizations where id=o),false) $$;
create function private.can_course(o uuid,c uuid, teaching boolean default false) returns boolean language sql stable security definer set search_path='' as $$
...
$$;
create function private.can_member(o uuid,u uuid) returns boolean language sql stable security definer set search_path='' as $$
...
create index memberships_user on public.memberships("userId","orgId");
create index courses_org on public.courses("orgId");
create index enrollments_student on public.enrollments("orgId","studentId");
create index lessons_course on public.lessons("orgId","courseId");
create index sessions_course on public.sessions("orgId","courseId");
create index submissions_org on public.submissions("orgId","studentId");
...
$$;
create function public.create_organization(p_name text,p_slug text) returns uuid language plpgsql security definer set search_path='' as $$
...
create function public.create_invitation(p_org uuid,p_email text,p_name text,p_role text,p_course uuid default null) returns text language plpgsql security definer set search_path='' as $$
