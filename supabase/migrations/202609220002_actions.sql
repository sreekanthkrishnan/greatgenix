...
begin
 ...
 if t='org-status' then
  ...
  when 'lesson' then
   ...
   insert into public.lessons(id,"orgId","courseId",title,duration,age,subject,"fileName") values((d->>'id')::uuid,p_org,c,trim(d->>'title'),(d->>'duration')::int,d->>'age',d->>'subject',d->>'fileName');
  when 'lesson-status' then
   ...
   perform private.require((p_action->>'status'='draft') or (p_action->>'status'='review' and l.status='draft' and l."mediaStatus"='ready') or (p_action->>'status'='published' and l.status='review' and l."mediaStatus"='ready' and (p_action->>'reviewed')::boolean),'Upload a ready video, then review before publishing');
