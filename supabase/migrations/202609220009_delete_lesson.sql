-- Migration 0009: Add delete-lesson support in apply_action RPC
...
begin
 ...
 if t='org-status' then
  ...
  when 'delete-lesson' then
   target:=(p_action->>'id')::uuid;
   select "courseId" into c from public.lessons where id=target and "orgId"=p_org;
   perform private.require(c is not null and private.can_course(p_org,c,true),'Permission denied to delete lesson');
   delete from public.completions where "orgId"=p_org and "lessonId"=target;
   delete from public.reports where "orgId"=p_org and "lessonId"=target;
   delete from public.lessons where id=target and "orgId"=p_org;
