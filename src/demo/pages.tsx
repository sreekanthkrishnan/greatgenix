...
import { ActionForm, type FormKind } from "./forms";
import { demoMedia } from "./integrations";
import {
  available,
  canAdmin,
  canSeeCourse,
...
function useScope() {
  ...
  const sessions = state.sessions
    .filter(
      (s) =>
        s.orgId === viewer.orgId && courses.some((c) => c.id === s.courseId),
    )
...
export function Dashboard() {
  ...
  const name =
    ...
    "there";
  const next = available(state, viewer, "live") ? sessions[0] : undefined;
    ...
    <>
        ...
        <section className="focus-card">
          ...
          <p>
            {teacher
              ...
              : `${completed} of ${lessons.length} available lessons marked complete in this demo.`}
          </p>
      ...
      {!courses.length && (
        <Empty
          ...
          text="An admin-assigned teacher can create a course and add a fictional learner."
        />
      )}
  ...
  if (!course)
    return (
      <Unavailable
        ...
        text="Switch back to your courses to see what is available in this demo role and organization."
      />
    );
          ...
          <div className="panel">
            ...
            {!roster.length && (
              <Empty
                ...
                text="Add fictional learners using the teacher + admin demo role."
              />
            )}
          </div>
        </>
...
export function Sessions({ id }: { id?: string }) {
  ...
  async function join() {
    setBusy(true);
    try {
      await demoMedia.joinSession(id!);
    } catch (error) {
      ...
      <>
        ...
        <div className="live-room">
          <div className="live-room-art">
            ...
            <p>
              Your live session will open here when a provider is connected.
            </p>
            <Badge tone="light">DEMO · PROVIDER NOT CONNECTED</Badge>
          </div>
          <div className="live-room-details">
            ...
            <p className="muted">
              Times follow your browser’s local time zone. No camera or
              microphone access is requested in this demo.
            </p>
  ...
  return (
    <>
      ...
      />
      <Notice>
        Sample schedules only. Live rooms and calendar notifications are not
        connected.
      </Notice>
        ...
        {sessions.map((s) => (
          <div className="schedule-card" key={s.id}>
            ...
            <div>
              ...
              <h3>{s.title}</h3>
              <p>
                {state.courses.find((c) => c.id === s.courseId)?.subject} ·
                Sample live session
              </p>
            </div>
      ...
      {!sessions.length && (
        <Empty
          ...
          text="Schedule a demo class to get started."
        />
      )}
...
export function Recordings({ id }: { id?: string }) {
  ...
  async function play() {
    try {
      await demoMedia.getPlayback(id!);
    } catch (error) {
      setMessage((error as Error).message);
    }
  }
    ...
    return (
      <>
        ...
        <div className="lesson-layout">
          <section>
            <div className={`video-placeholder ${course.color}`}>
              <CourseArt color={course.color} large />
              <button
                className="play-button"
                aria-label="Check recording playback"
                onClick={play}
              >
              ...
              <div className="video-caption">
                ...
                <Badge tone="light">No video connected</Badge>
              </div>
            </div>
            {message && (
              <div className="inline-error" role="status">
            ...
            <div className="lesson-description">
              ...
              <p>{course.description}</p>
              <Notice>
                This demo has lesson metadata, not playable media. No video has
                been uploaded. Completion and review actions below are
                simulations.
              </Notice>
            </div>
          </section>
          <aside className="panel lesson-sidebar">
            ...
            {lesson.fileName && (
              <small className="file-name">
                Local filename: {lesson.fileName}
              </small>
            )}
            {teacher ? (
              <>
                  ...
                  <Button
                    onClick={() =>
                      act(
                        ...
                        "Demo draft moved to review. No media was processed.",
                      )
                    }
                  >
                    <ArrowRight size={16} />
                    ...
                    <Button
                      ...
                      onClick={() =>
                        act(
                          ...
                          "Demo lesson published locally. Real content review remains required.",
                        )
                      }
                    >
                      <Check size={16} />
                ...
                {lesson.status === "published" && (
                  <>
                    <CheckLabel>Visible in the student demo</CheckLabel>
                    <Button
                      variant="secondary"
                      onClick={() => {
                        setReviewed(false);
                        act(
                          ...
                          "Lesson withdrawn in the local demo.",
                        );
                      }}
                    >
                      Withdraw to draft
    ...
    <>
            ...
            <section className="assessment-card" key={a.id}>
              ...
              {teacher ? (
                <div className="submission-list">
                  ...
                  {!subs.length && (
                    <p className="muted">
                      No submissions yet. Try the student demo to submit an
                      answer.
                    </p>
                  )}
                </div>
                ...
                <div className="assessment-footer">
                  ...
                  <Button variant="secondary" onClick={() => setSelected(a.id)}>
                    {sub ? "View / revise demo work" : "Open assessment"}
      ...
      {assignment && (
        <Modal
          ...
          description="Your answer is saved to this browser demo only."
        ...
        >
            ...
            onSubmit={(e) => {
              ...
              if (
                act(
                  ...
                  "Demo answer submitted locally. No work was sent to a teacher.",
                )
              )
                setSelected(null);
            }}
          >
            ...
            {own?.published && (
              <Notice>
                Revising a demo submission clears its published grade until
                reviewed again.
              </Notice>
            )}
            <div className="form-actions">
              ...
              <Button type="submit">
                Submit in demo <ArrowRight size={16} />
              </Button>
            </div>
          </form>
        </Modal>
            ...
            onSubmit={(e) => {
              ...
              if (
                act(
                  ...
                  "Feedback published to the local student demo.",
                )
              )
                setReviewId(null);
            }}
  ...
  if (!teacher)
    return (
      <Unavailable
        ...
        text="Attendance editing is available to teachers in this demo."
      />
    );
    ...
    <>
      ...
      {session && course ? (
        <AttendanceRegister
          ...
          onSave={(values) =>
            act(
              { type: "attendance", sessionId, values },
              "Attendance saved in this browser demo only.",
            )
          }
        />
      ) : (
...
function AttendanceRegister({
  ...
  sessionId: string;
  studentIds: string[];
  onSave: (values: Record<string, AttendanceStatus>) => boolean;
}) {
  const { state } = useDemo();
    ...
    <>
      <div className="panel">
        ...
        {!members.length && (
          <Empty
            ...
            text="An admin-assigned teacher can add fictional learners to this course."
          />
        )}
      </div>
      <div className="save-bar">
        <span>
          ...
          unmarked {dirty ? "· Unsaved demo changes" : ""}
        </span>
        <Button
          disabled={!dirty}
          onClick={() => {
            if (onSave(values)) setDirty(false);
          }}
        >
          Save demo attendance <Check size={16} />
        </Button>
      </div>
    </>
  );
  ...
  if (!canAdmin(viewer.role))
    return (
      <Unavailable
        ...
        text="Only explicitly assigned teacher-admins have this demo view. Ordinary teachers do not receive administrator permissions."
      />
    );
  return (
    <>
      <PageHeading
        ...
        description="A few thoughtful settings for your shared learning space."
      />
      <div className="settings-grid">
        <section className="panel">
          <h2>Organization profile</h2>
          <p className="muted">One organization, wherever learning happens.</p>
          <form
            className="form"
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            ...
            <Notice>
              You have an explicit teacher-admin assignment in this demo.
              Feature changes remain in the platform preview.
            </Notice>
        ...
        <section className="panel">
          ...
          <p className="muted">
            Feature access and people’s permissions are separate.
          </p>
          {(Object.entries(featureLabels) as [Feature, string][]).map(
            ([key, label]) => (
              <div className="setting-row" key={key}>
                <span>{label}</span>
                <Badge tone={org.features[key] ? "sage" : "neutral"}>
                  {org.features[key] ? "Enabled" : "Not enabled"}
                </Badge>
              </div>
            ),
          )}
      ...
      <section className="panel report-panel">
        <SectionHeading title="Demo content reports" />
        {state.reports.filter((r) => r.orgId === viewer.orgId).length ? (
          state.reports
            .filter((r) => r.orgId === viewer.orgId)
            .map((r, i) => (
              <div className="report-row" key={i}>
                ...
                <a className="text-link" href={`#/recordings/${r.lessonId}`}>
                  Review lesson <ArrowUpRight size={14} />
                </a>
              </div>
            ))
        ) : (
          <p className="muted">
            No reports in this browser demo. Student reports appear here for
            review.
          </p>
        )}
      </section>
      {form && <ActionForm kind="member" close={() => setForm(false)} />}
    </>
  );
}
...
export function Organizations() {
  ...
  if (viewer.role !== "super-admin")
    return (
      <Unavailable
        ...
        text="Use the clearly labeled platform demo persona to preview these controls."
      />
    );
  ...
  return (
    <>
      ...
      />
      <Notice>
        Platform demo only. All usage below is fictional local inventory, not
        measured provider billing or subscription data.
      </Notice>
      <div className="platform-grid">
        ...
        <section className="panel org-detail">
          ...
          <div className="mini-stats">
            <div>
              <strong>
                {state.courses.filter((c) => c.orgId === org.id).length}
              </strong>
              <span>Demo courses</span>
            </div>
            <div>
              <strong>
                {state.members.filter((m) => m.orgId === org.id).length}
              </strong>
              <span>Demo people</span>
            </div>
            <div>
              <strong>
                {state.lessons.filter((l) => l.orgId === org.id).length}
              </strong>
          ...
          <p className="muted">
            Switches change this organization’s local demo only.
          </p>
          {(Object.entries(featureLabels) as [Feature, string][]).map(
            ([key, label]) => (
              <div className="setting-row" key={key}>
                ...
                <button
                  ...
                  onClick={() =>
                    act(
                      { type: "feature", orgId: org.id, feature: key },
                      `${label} changed for ${org.name} in the demo only.`,
                    )
                  }
                >
                  <span />
          ...
          <p className="fine-print">
            Disabling a feature hides its demo views; existing local records are
            ...
            <Button
              ...
              onClick={() =>
                act(
                  ...
                  `${org.name} ${org.active ? "suspended" : "restored"} in the local demo.`,
                )
              }
            >
              {org.active
                ? "Suspend demo organization"
                : "Restore demo organization"}
            </Button>
          </div>
        </section>
      </div>
...
export function DemoAccess({ enter }: { enter: () => void }) {
  return (
    <div className="access-page">
      <div className="access-emblem">
        <GraduationCap size={32} />
      </div>
      <span className="eyebrow">GREAT GENIX</span>
      <h1>
        A little curiosity.
        <br />A world of possibility.
      </h1>
      <p>
        Explore the teacher and student experience with fictional data. No
        account or password needed.
      </p>
      <Button onClick={enter}>
        Enter the local demo <ArrowRight size={17} />
      </Button>
      <Notice>
        Real sign-in, recovery and invitations will use Supabase Auth. They are
