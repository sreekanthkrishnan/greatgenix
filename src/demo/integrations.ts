...
export function createConfiguredSupabaseClient() {
  ...
  if (!url || !key)
    throw new Error(
      "Supabase is not configured. The local demo remains available.",
    );
...
export const demoMedia: MediaGateway = {
  async getPlayback() {
    throw new Error(
      "Recorded video is not connected. No video has been loaded or streamed.",
    );
  },
  async joinSession() {
    throw new Error(
      "Live classes are not connected. No room was created and your camera and microphone were not accessed.",
    );
  },
  async authorizeUpload() {
    throw new Error(
      "Video uploads are not connected. Only lesson metadata can be saved in this local demo.",
    );
  },
};
